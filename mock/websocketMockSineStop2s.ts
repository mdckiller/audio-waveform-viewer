...
export const MOCK_LABEL = "sine-8s-stop-2s";

export class Sine8sStop2sMockWebSocket extends EventTarget {
  ...
  private readonly stopEveryMs: number;

  constructor(sampleIntervalMs = 16, periodMs = 8000, stopEveryMs = 2000) {
    super();
    this.sampleIntervalMs = sampleIntervalMs;
    this.periodMs = periodMs;
    this.stopEveryMs = stopEveryMs;
  }
  ...
  private tick(): void {
    ...
    while (nowPerf >= this.nextDue && emitted < 512) {
      ...
      this.emitDataIfRunningPhase(elapsedMs);
  ...
  private emitDataIfRunningPhase(elapsedMs: number): void {
    const timestamp = this.originTimestamp + elapsedMs;

    const phaseIndex = Math.floor(elapsedMs / this.stopEveryMs);
    const paused = phaseIndex % 2 === 1;
    if (paused) {
      return;
    }
